    ...
    <div class="card-body">
                ...
                <div class="card bg-warning-subtle border-warning">
                    <div class="card-body py-2 px-3">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="badge bg-warning rounded-pill">{{ $stats['maintenance']??0 }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mb-3">
            ...
            <div class="form-check form-switch d-inline-block ms-3">
                <input class="form-check-input" type="checkbox" id="showOnlyFree" {{ request('free_only')?'checked':'' }}>
        ...
        <div class="slots-grid row g-3" id="slotsContainer">
            @foreach($schedule->slots->sortBy('slot_number') as $slot)
                <div class="col-md-4 col-lg-3 slot-item" data-status="{{ $slot->status }}" data-slot-id="{{ $slot->id }}">
                    <div class="card h-100 slot-card {{ $slot->status }}" 
                        ...
                        <div class="card-body py-3">
                            ...
                            @if($slot->status === 'booked' && $slot->appointment)
                                <div class="small">
                                    <div class="fw-medium text-truncate" title="{{ $slot->appointment->patient->name ?? 'N/A' }}">
                            ...
                            <div class="btn-group w-100 btn-group-sm" role="group">
                                @if($slot->status === 'available')
                                    ...
                                    <button type="button" class="btn btn-outline-primary toggle-slot" 
                                            data-action="assign" title="Gán appointment thủ công"
                                ...
                                @elseif($slot->status === 'booked')
                                    <a href="{{ route('admin.schedules.slots', $slot->appointment_id) }}" 
                                       class="btn btn-outline-primary" title="Xem chi tiết appointment">
                                        👁️
                                    </a>
                                    <button type="button" class="btn btn-outline-danger toggle-slot" 
        ...
        @if($schedule->slots->isEmpty())
            <div class="text-center py-5 text-muted">
                ...
                <form action="{{ route('admin.schedules.generate-slots', $schedule->id) }}" method="POST" class="d-inline">
                    @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="symptoms" class="form-control" rows="2" 
                                  placeholder="Mô tả ngắn về lý do khám..."></textarea>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.toggle-slot').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            const endpoints = {
                block: `/admin/slots/${slotId}/block`,
                unblock: `/admin/slots/${slotId}/unblock`,
                cancel: `/admin/slots/${slotId}/cancel-appointment`
            };
            
            if (!endpoints[action]) return;
            
            if (action === 'cancel' && !confirm('Hủy appointment này sẽ giải phóng slot và thông báo cho bệnh nhân. Tiếp tục?')) {
                return;
            }
