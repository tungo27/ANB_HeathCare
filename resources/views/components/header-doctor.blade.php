<header class="sticky top-0 z-50 w-full bg-white border-b flex items-center justify-between px-8 py-4 shadow-sm">
    ...
    <nav class="d-none d-md-flex align-items-center gap-4">
        ...
        <a href="{{ route('doctor.dashboard') }}"
            class="{{ $navClass }} {{ request()->routeIs('doctor.dashboard') ? $activeClass : $inactiveClass }}">
            Lịch trực của tôi
        </a>

        <a href="{{ route('doctor.appointments') }}"
            class="{{ $navClass }} {{ request()->routeIs('doctor.appointments') ? $activeClass : $inactiveClass }}">
            Lịch hẹn khám
        </a>

        <a href="#" class="{{ $navClass }} {{ $inactiveClass }}">Bệnh nhân của tôi</a>
    </nav>
    ...
    <div class="dropdown">
        <button
            ...
            <div
                ...
                {{ substr(Auth::user()->full_name ?? (Auth::user()->name ?? 'B'), 0, 1) }}
            </div>
            <span class="fw-bold text-base text-gray-700">
                Bác sĩ {{ Auth::user()->full_name ?? Auth::user()->name }}
            </span>
        </button>
